...
// Modul for doing leg animation as well as sending the chains to the different LockGoard listeners
...
CallAnim( string szMsg, key keyID )
{
    ...
    else
    {
        ...
        if (nIdx != -1 )
        {
            ...
            if (szAnim=="*none*")
                // Cleo: Added to allow only chains, if name of anim is "*none*"
            {
        ...
        else if ( auth == DIALOG_RESPONSE)
        {
            if (id==g_keyDialogID)
            {
                ...
                else if (~llListFindList(g_lstLocks, [message]))
                {
                    if (message=="*Stop*")
                    {
                        llMessageLinked(LINK_THIS, COMMAND_NOAUTH, "l:Stop", AV);
                    }
                    else
                    {
                        llMessageLinked(LINK_THIS, COMMAND_NOAUTH, "l:"+message, AV);
                    }
                    //CallAnim(message, AV);
                    DoMenu(AV);
                }
