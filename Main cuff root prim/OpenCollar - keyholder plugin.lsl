...
integer kh_return_on_timer = TRUE; // Return key on timer expire?
integer kh_auto_return_timer = TRUE; // Start a timer for key return?
...
TakeKey(key avatar, integer auth, integer remote)
{
    ...
    if (kh_auto_return_timer && kh_auto_return_time)
    {
        ...
        llOwnerSay("Setting key return timer for " + (string)hours + ":" + (string)minutes ) ;
